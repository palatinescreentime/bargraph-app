...
const PRICE_TIERS = [
  { id: 'budget', min: 0,  max: 7,  rep: 5,  sub: '$4–7',  label: '$',   r: 6  },
  { id: 'mid',    min: 8,  max: 11, rep: 9,  sub: '$8–11', label: '$$',  r: 9  },
  { id: 'pricey', min: 12, max: 99, rep: 13, sub: '$12+',  label: '$$$', r: 12 },
];

function getPriceTier(price) {
  return PRICE_TIERS.find(t => price >= t.min && price <= t.max) || PRICE_TIERS[PRICE_TIERS.length - 1];
}
...
export default function BarGraph({ city, bars = [], categories = {} }) {
  ...
  function barVisible(b) {
    ...
    const priceOk = activeTiers.size === 0 ||
      PRICE_TIERS.some(t => activeTiers.has(t.id) && b.price >= t.min && b.price <= t.max);
    if (!priceOk) return false;
    return true;
  }

  const filteredBars = useMemo(() => bars.filter(barVisible), [bars, activeSubareaLabel, activeCats, activeTiers]);
  const hasFilter = activeSubarea || activeCats.length > 0 || activeTiers.size > 0;

  const subareaCount = useMemo(() => {
    const c = {};
    bars.forEach(b => { c[b.subarea] = (c[b.subarea] || 0) + 1; });
    return c;
  }, [bars]);
  ...
  function handleSubareaClick(slug) {
    if (activeSubarea === slug) window.location.hash = '';
  ...
  const usedCats = useMemo(
    () => [...new Set(bars.map(b => b.cat))].filter(c => categories[c]),
    [bars, categories]
  );

  const isAdmin = currentUser?.email === ADMIN_EMAIL;
  const truncatedEmail = currentUser?.email
    ? currentUser.email.length > 22 ? currentUser.email.slice(0, 20) + '…' : currentUser.email
    : null;

  const submitUrl = currentUser
    ? `/submit?city=${city?.id || ''}`
    : '/auth';
  const submitLabel = currentUser
    ? `＋ Add a venue to ${city?.name || 'this city'}`
    ...
    <div style={{ background: bg, minHeight: '100vh', color: fg, fontFamily: 'sans-serif', transition: 'background 0.3s' }}>
      <style>{`
        * { box-sizing: border-box; }
        ...
        .tier-btn:hover { color: #aaa; }
        .tier-btn.active { background: rgba(255,255,255,0.07); border-color: #aaa; color: #fff; }
        .bottom-sheet {
        ...
        .add-venue-link {
          color: #4ab8e8; font-size: 0.78rem; text-decoration: none;
          padding: 0.3rem 1.25rem 0.5rem; display: inline-block;
          opacity: 0.7; transition: opacity 0.15s;
        }
        .add-venue-link:hover { opacity: 1; }
      `}</style>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          {currentUser ? (
            <>
              ...
              <button onClick={signOut} className="auth-link">Sign out</button>
            </>
          ) : (
      ...
      <div style={{ padding: '0.5rem 1.25rem', borderBottom: `1px solid ${dark ? '#111' : '#eee'}` }}>
        {/* Categories */}
        <div style={{ display: 'flex', gap: 6, overflowX: 'auto', flexWrap: 'wrap', marginBottom: 6 }}>
          {usedCats.map(cat => {
            ...
            return (
              <button
                ...
                style={{ color: isActive ? color : '#888', borderColor: isActive ? color : 'transparent' }}
          ...
          {(activeCats.length > 0 || activeTiers.size > 0) && (
            <button className="cat-btn" style={{ color: '#f59e0b' }} onClick={clearAll}>
              Clear all
            </button>
          )}
        </div>
        ...
        <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
          <span style={{ fontSize: '0.7rem', color: '#444', textTransform: 'uppercase', letterSpacing: '0.08em', marginRight: 2 }}>
            Price
          </span>
          {PRICE_TIERS.map(t => {
            const isActive = activeTiers.has(t.id);
            return (
              <button
                key={t.id}
              ...
              >
                <svg width={t.r * 2} height={t.r * 2} style={{ flexShrink: 0 }}>
                  <circle
                    cx={t.r} cy={t.r} r={t.r - 1}
      ...
      {/* Add venue CTA */}
      <div>
        <a href={submitUrl} className="add-venue-link">
          {submitLabel}
        </a>
      </div>
      ...
      <div ref={svgRef} style={{ width: '100%', maxWidth: 700, margin: '0 auto', position: 'relative' }}>
        ...
        >
          {[-4, -3, -2, -1, 0, 1, 2, 3, 4].map(v => (
            <g key={v}>
              <line x1={toSVG(v, svgSize)} y1={PAD} x2={toSVG(v, svgSize)} y2={svgSize - PAD} stroke={gridColor} strokeWidth={1} />
              <line x1={PAD} y1={toSVG(v, svgSize)} x2={svgSize - PAD} y2={toSVG(v, svgSize)} stroke={gridColor} strokeWidth={1} />
            </g>
          ))}
          <line x1={PAD} y1={toSVG(0, svgSize)} x2={svgSize - PAD} y2={toSVG(0, svgSize)} stroke={axisColor} strokeWidth={1.5} />
          <line x1={toSVG(0, svgSize)} y1={PAD} x2={toSVG(0, svgSize)} y2={svgSize - PAD} stroke={axisColor} strokeWidth={1.5} />
          ...
          {bars.map(bar => {
            ...
            return (
              ...
              >
                <circle r={tier.r} fill={color} opacity={dimmed ? 0.1 : 0.85} />
                {(isHovered || (hasFilter && isFiltered)) && (
                  <text y={-tier.r - 4} fontSize={9} fill={fg} textAnchor="middle" style={{ pointerEvents: 'none' }}>
                    {bar.name}
                  </text>
                )}
              </g>
        ...
        {hovered && (
          ...
          >
            ...
            <div style={{ marginTop: 6, display: 'flex', gap: 8, fontSize: '0.75rem', flexWrap: 'wrap' }}>
              ...
              <span style={{ color: '#888' }}>{getPriceTier(hovered.price).sub}</span>
      ...
      {selected && (
        <div className="bottom-sheet">
          ...
          <div style={{ display: 'flex', gap: 10, fontSize: '0.8rem', flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            <span style={{ color: '#aaa' }}>{getPriceTier(selected.price).sub}</span>
